...
function findLastContentBefore(frame: FrameNode, beforeY: number): number {
  let latest = -1;
  function search(node: SceneNode, absY: number): void {
    if (absY >= beforeY) return;
    const bottom = absY + getH(node);
    if ('children' in node && (node as ChildrenMixin).children.length > 0) {
      for (const child of (node as ChildrenMixin).children) {
        search(child, absY + getY(child));
      ...
      if (effectiveBottom > latest) latest = effectiveBottom;
    }
  }
  for (const child of frame.children) search(child, getY(child));
  return latest;
}
